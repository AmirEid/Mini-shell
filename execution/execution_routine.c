...
/*   Updated: 2024/07/27 17:54:13 by anomourn         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static int	ft_get_number_of_redirections(t_list *tokens)
{
	...
	while (current != NULL)
	{
		...
		if (tokendata->type == META_REDIR_IN || tokendata->type == META_REDIR_OUT || tokendata->type == META_APPEND || tokendata->type == META_HEREDOC)
			num_redirs++;
...
static void	execute_redirections(t_list *token, t_list *env, t_data *data)
{
	...
	while (current != NULL && tokendata->type != META_PIPE)
	{
		if (tokendata->type == META_REDIR_IN)
			...
			ft_heredoc(current->next, env, data);
		current = current->next;
		if (current != NULL)
			tokendata = (t_tkn_data *)current->content;
	}
